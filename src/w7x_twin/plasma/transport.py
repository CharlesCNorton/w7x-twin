...
#: Perpendicular box length the ion-scale calibration runs carry. A run at another y0
#: resolves other binormal wavenumbers, so its flux answers to a different linear drive.
CALIBRATION_Y0 = 15.0
    ...
    def read(cls, path):
        ...
        for point in stored.get("points", []):
            ...
            if point.get("seconds") is None:
                continue
            # An electron-scale box carries a different set of binormal wavenumbers, so its
            # flux stands against a different drive and not on this curve.
            if float(point.get("y0", CALIBRATION_Y0)) != CALIBRATION_Y0:
                continue
