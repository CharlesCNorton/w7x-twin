...
electron channel carried as its own curve at 0.30 to 0.42 of the ion flux.

### What the perpendicular resolution is worth

The box the response was measured in carries binormal wavenumbers only to ky = 0.333, where
ion-scale saturation is conventionally taken to need 1.5 to 2, so growing it at fixed y0
raises that ceiling rather than enlarging the domain: the added modes are a sink the smaller
box did not have, and the flux falls at every step. Five rungs on s = 0.81 at a/L_T = 4.5,
all at the same trace length and parallel resolution:

| Box | ky_max | Q_i [gyro-Bohm] | Against the rung below |
|---|---|---|---|
| 24 × 16 | 0.33 | 15.46 | — |
| 48 × 32 | 0.67 | 8.051 | 1.92 |
| 72 × 48 | 1.00 | 7.708 | 1.04 |
| 96 × 64 | 1.40 | 10.10 | 0.76 |
| 108 × 72 | 1.53 | 10.49 | 0.96 |

The sequence is not monotone. It passes through a minimum at ky_max = 1.00 and returns, so
the 1.04 step there is the turnaround and not convergence; the converged pair is 1.40 and
1.53, four per cent apart, and the default box overstates that limit by 1.50. On s = 0.25
and s = 0.49 the flux is still falling at ky_max 1.53, and the same fall appears at the
gradients the transport channel reads: at s = 0.25, a/L_T = 2.5 the default box gives 137.2
against 26.42 at ky_max 1.00, a factor of 5.19, against 5.51 for the same pair of boxes at
a/L_T = 4.5.

The pellet-level density gradient at s = 0.49 is decided by the same resolution. It comes
apart at ky_max 0.33 and 0.67, at the default timestep and at half of it, and saturates from
1.00 upward: 15.06, then 8.541 at 1.40 and 6.84 at 1.53, whose successive differences of
6.52 and 1.70 extrapolate to 6.2. A divergence there is the grid scale accumulating energy
it cannot pass on, not a timestep.

The electron-scale box, which puts ky 3 to 15 inside it, returns 0.004 gyro-Bohm against
4.72 at the same surface and gradient, so the ky = 15 row of the growth-rate grid carries no
saturated flux worth relating to the ion-scale curve.
