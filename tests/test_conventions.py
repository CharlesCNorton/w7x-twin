...
def test_no_tracked_file_carries_a_home_directory():
    """A committed file names no account and no machine it was produced on."""
    import re
    import subprocess
    from pathlib import Path
    ...
    home = re.compile(r"/home/[^/\s\"']+|/Users/[^/\s\"']+|[A-Za-z]:\\?Users\\?[^\\s\"']+")
    offenders = []
    for name in listing.stdout.split():
        path = root / name
        if path.suffix in {".png", ".jpg", ".nc", ".npz"} or not path.is_file():
            continue
        ...
        try:
            ...
            continue
        for match in home.findall(text):
            # The pattern itself is written down in this repo, and a real path
            # carries no character class, so a match holding one is the source
            # of the rule rather than a leak.
            if "[^" in match:
                continue
            offenders.append(f"{name}: {match}")
    assert not offenders, "home directories in tracked files: " + "; ".join(offenders[:8])


def test_every_generated_input_is_named_by_the_roster():
    """An input the package makes for itself is declared, so an absent one is known."""
    import re
    from pathlib import Path

    from w7x_twin.analyses import data as data_analysis

    root = Path(__file__).resolve().parents[1]
    named = set()
    for path in (root / "src").rglob("*.py"):
        named.update(
            re.findall(r'coils_file="([A-Za-z_0-9.\-]+)"', path.read_text(encoding="utf-8"))
        )
    assert named, "no coils files found to check"
    known = set(data_analysis.SOURCES) | set(data_analysis.GENERATED)
    unknown = sorted(name for name in named if name not in known)
    assert not unknown, f"named but neither fetched nor declared generated: {unknown}"
